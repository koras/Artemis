...
{
    /// <summary>
    /// Recalculates connected components of the built Oxygen pipe graph.
    /// </summary>
    public sealed class OxygenNetworkService
    {
        ...
        public OxygenNetworkService(GridState gridState)
        {
        ...
        public void Recalculate()
        {
            ...
            for (int y = 0; y < _gridState.Height; y++)
            {
                for (int x = 0; x < _gridState.Width; x++)
                {
                    ...
                    if (!cell.HasOxygen || componentByCell.ContainsKey(cellPos))
                    {
                        continue;
                    }
            ...
            for (int y = 0; y < _gridState.Height; y++)
            {
                for (int x = 0; x < _gridState.Width; x++)
                {
                    var cellPos = new Vector2Int(x, y);
                    Cell cell = _gridState.GetCell(x, y);
                    cell.OxygenNetworkId = componentByCell.TryGetValue(cellPos, out int id) ? id : 0;
            ...
            {
                Debug.Log($"[OxygenNetwork] Pipe components changed: {_lastComponentCount} -> {componentCount}.");
